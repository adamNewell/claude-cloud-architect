...
Complete command reference for `@living-architecture/riviere-cli`. Used in **Phases 3–6** to build and query the architecture graph.
...
```bash
riviere builder <command>   # build / modify the graph
riviere query <command>     # read / query the graph
riviere extract <command>   # extract components from source
```
...
**Component ID format:** `{domain}:{module}:{type-lowercase}:{name}`
...
| Phase              | Commands Used                                                                                  |
| ------------------ | ---------------------------------------------------------------------------------------------- |
| Phase 3 — Extract  | `init`, `add-source`, `add-domain`, `define-custom-type`, `add-component`, `component-summary` |
| Phase 4 — Link     | `component-checklist`, `link`, `link-http`, `link-external`                                    |
| Phase 5 — Enrich   | `component-checklist --type DomainOp`, `enrich`                                                |
| Phase 6 — Validate | `check-consistency`, `validate`, `finalize`                                                    |
| Any phase          | `query domains`, `query components`, `query trace`, `query orphans`                            |
